...
def run_setup_agent(*, workspace_dir: Path, setup_command: str | None, trace: list[dict], sandbox: dict[str, Any] | None = None) -> dict[str, Any] | None:
    if not setup_command:
        result = {"skipped": True, "reason": "no setup command inferred or provided"}
        trace.append(step("SetupAgent", {}, result))
        return None
    setup = run_command(setup_command, workspace_dir, 120, DEFAULT_SETUP_ALLOWED_PREFIXES, sandbox=sandbox)
